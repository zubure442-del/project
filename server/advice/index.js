...
На входе JSON:
- consequence — главное, что сейчас происходит с телом; часто это оценка из приложения («Организм», «Сон») и то, что её тянет;
- root_cause — первопричина из истории кольца и как она с этим связана.

Ответ — ровно два предложения, вместе примерно 90–130 символов, не короче 80:
1. Первое — consequence своими словами. Если там названа оценка («Организм», «Сон»), назови её так же.
2. Второе — root_cause своими словами, как дружеская догадка, и обязательно сохрани связь — почему это влияет («похоже, это после…, и тело…»).
...
/**
 * На последней попытке «коротко», «похоже на прежнее» и «потерял предмет» — не повод отдавать шаблонную
 * фразу движка: живой пересказ лучше (владелец 26.09: «почему он выдаёт ошибки»).
 */
const SOFT_MIN_CHARS = 40;
const SOFT_PROBLEMS = new Set(['short', 'repeat', 'lost']);
/** Плашка на главном экране маленькая: длиннее — до предела карточки в приложении (`AI_ADVICE_MAX_CHARS`). */
...
/** Модель просим короче (130): так ответ почти никогда не упирается в предел проверки. */
...
/**
 * Шаблонная вводная в начале предложения — убираем сами, а не отклоняем ответ (владелец 26.09: «почему он
 * выдаёт ошибки»): «Возможно, сказалась ночь» → «Сказалась ночь».
 */
const HEDGE_PREFIX = /(^|[.!?…]\s+)(возможно|вероятно|скорее всего|наверное|может быть|должно быть)\s*,?\s+([а-яёa-z])/gi;

/** Текст ответа: без «Лис:» в начале, кавычек вокруг, шаблонных вводных и лишних пробелов. */
function cleanAnswer(raw) {
  return String(raw)
    ...
    .replace(/\s+/g, ' ')
    .trim()
    .replace(HEDGE_PREFIX, (_, before, _hedge, letter) => `${before}${letter.toUpperCase()}`);
}
...
const PROBLEM_TEXT = {
  ...
  long: 'длиннее 140 символов — сократи',
...
async function handler(event, context) {
  ...
  for (let attempt = 0; attempt < 2; attempt++) {
    ...
    messages.push({ role: 'assistant', text: result.text }, { role: 'user', text: `Ответ не подходит: ${PROBLEM_TEXT[problem]}. Напиши заново по правилам — ровно два предложения, примерно 90–130 символов: первое — consequence, второе — root_cause.` });
  }

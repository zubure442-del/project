...
describe('СИНТЕТИЧЕСКИЕ: облачная функция «Мнения Лиса»', () => {
  ...
  it('системный запрос: пересказать факт и причину разговорным языком; все показатели можно называть; стоп-лист', () => {
    ...
    expect(prompt).toContain('consequence — главное, что сейчас происходит с телом');
    expect(prompt).toContain('сохрани связь');
    expect(prompt).toContain('root_cause — первопричина из истории кольца');
    expect(prompt).toContain('Ответ — ровно два предложения, вместе примерно 90–130 символов, не короче 80');
  ...
  it('ответ не прошёл проверку — один повтор с причиной; и он мимо — связка движка вместо 502', async () => {
    ...
    const bad = 'Сейчас движения почти нет, а пульс в покое заметно выше. Не хватило глубокого сна за две ночи, и тело не восстановилось.';
    const bodies = [];
    ...
    expect(bodies[1].messages.slice(-2)).toEqual([
      ...
      { role: 'user', text: 'Ответ не подходит: без канцелярита («в покое», «наблюдается», «Причина —») — скажи простыми словами. Напиши заново по правилам — ровно два предложения, примерно 90–130 символов: первое — consequence, второе — root_cause.' },
    ]);

    // «Возможно, …» в начале предложения — не ошибка: вводную функция убирает сама (владелец 26.09).
    answers = ['Сейчас движения почти нет, а пульс заметно выше обычного. Возможно, не хватило глубокого сна за две ночи подряд.'];
    const hedged = await fn.handler(event(await readSample()), context);
    expect(JSON.parse(hedged.body).text).toBe('Сейчас движения почти нет, а пульс заметно выше обычного. Не хватило глубокого сна за две ночи подряд.');

...
describe('СИНТЕТИЧЕСКИЕ: облачная функция «Мнения Лиса»', () => {
  ...
  it('ответ: до 130 символов, два предложения, без цифр, команд, вводных «возможно», выдумок, тавтологий и повторов', () => {
    ...
    expect(fn.answerProblem('Давление держится выше вашего обычного. Похоже, ночь вышла короче, чем нужно телу.')).toBeNull();
    expect(fn.answerProblem('Сил к вечеру меньше. Похоже, сахар сегодня скачет сильнее обычного из-за перекусов.')).toBeNull();
    expect(fn.answerProblem('Сон был долгим, а сил мало. Кажется, ночью кислород в крови проседал ниже обычного.')).toBeNull();
    expect(fn.answerProblem('Давление держится выше обычного. Похоже, это начинается гипертония после плохой ночи.')).toBe('forbidden');
    // Канцелярит и сленг.
    expect(fn.answerProblem('Тело сейчас в покое. Похоже, ночь была короче обычной и сил меньше.')).toBe('clerical');
    expect(fn.answerProblem('Пульс держится выше обычного. Причина — поздний ужин накануне вечером.')).toBe('clerical');
    expect(fn.answerProblem('Мотор шпарит без остановки. Похоже, ночью кислород проседал ниже обычного.')).toBe('slang');
    expect(fn.answerProblem('Пульс выше обычного после обеда. Иди пройдись, и всё пройдёт само собой.')).toBe('command');

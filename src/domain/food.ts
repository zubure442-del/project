...
/** Фиксированный текст кнопки «i» рядом со счётчиком аутофагии. */
export const AUTOPHAGY_INFO = {
  title: 'Что такое аутофагия',
  text:
    'Аутофагия — обычная «уборка» внутри клеток: когда с последней еды проходит много времени, организм ' +
    'начинает разбирать и пускать в дело собственный внутренний мусор. Считается, что заметной она ' +
    'становится примерно после 12 часов без еды — поэтому счётчик начинает идти только с этого момента.\n\n' +
    'Это не награда и не задача на день: и длинная пауза, и короткая — нормальные состояния, разные для ' +
    'разных дней. «0 часов» значит только то, что перерыв между едой был короче двенадцати часов; ' +
    'исправлять тут нечего. Если хочется, чтобы пауза была длиннее, проще поужинать пораньше, чем ' +
    'пропускать завтрак.\n\n' + NOT_MEDICAL_DEVICE,
} as const;
...
/** «3 часа 40 минут аутофагии по итогам ночного отдыха» либо «0 часов». */
export function autophagyText(minutes: number): string {
  ...
  return `${parts.join(' ')} аутофагии по итогам ночного отдыха`;
}
